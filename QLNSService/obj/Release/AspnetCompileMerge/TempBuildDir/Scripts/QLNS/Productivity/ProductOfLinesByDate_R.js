...
GPRO.namespace('LCDProductivitiesOfLinesByDate');
GPRO.LCDProductivitiesOfLinesByDate = function () {
    var Global = {
        UrlAction: {
    ...
    function BuildChart(datas) {
        ...
        var settings = {
            title: 'Năng suất các Chuyền - Ngày : ' + parseJsonDateToDate(datas[0].Date).toLocaleDateString(),
            description: '',
            showLegend: true,
            source: data,
            padding: {
        ...
        // setup the chart
        $('#jqxChartDay').css('height', parseInt($('[height]').html()));
        $('#jqxChartDay').jqxChart(settings); 
    }
}


$(document).ready(function () {
    var LCDProductivitiesOfLinesByDate = new GPRO.LCDProductivitiesOfLinesByDate();
    LCDProductivitiesOfLinesByDate.Init();
})