...
GPRO.namespace('LCDNSOfLineByHours_Output');
GPRO.LCDNSOfLineByHours_Output = function () {
    var Global = {
        UrlAction: {
    ...
    function GetProductivity() {
        $.ajax({
            ...
            type: 'POST',
            data: JSON.stringify({ 'lineIds': Global.Data.LineId, 'date': new Date(), 'isOneLine': true, 'IsProductOutput': true }),
    ...
    function BuildChart(datas) {
        ...
        var settings = {
            title: "Thành phẩm thoát chuyền của Chuyền : " + datas.LineName + ' - Ngày : ' + parseJsonDateToDate(datas.Date).toLocaleDateString(),
            description: '',
            showLegend: true,
            source: data,
            padding: {
        ...
        // setup the chart
        $('#jqxChartHours_Output').css('height', parseInt($('[height]').html()));
        $('#jqxChartHours_Output').jqxChart(settings);
    }
}


$(document).ready(function () {
    var LCDNSOfLineByHours_Output = new GPRO.LCDNSOfLineByHours_Output();
    LCDNSOfLineByHours_Output.Init();
})