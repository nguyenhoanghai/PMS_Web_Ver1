...
GPRO.LCDNS_Coll_N = function () {
                ...
                case 'tblpanelBody':
                    switch (parseInt(v.RowInt)) {
                        ...
                        case 4:
                            ...
                            break;
                        case 5:
                            style.append('#tb_pro_collec .forter{ height:' + v.SizePercent + '%}');
                            break;
                    }
                    break;
                case 'tblpanelHeader':
            ...
            switch (v.TableLayoutPanelName.trim()) {
                case 'tblPanelHeader':
                    switch (parseInt(v.Position)) {
                        case 1: style.append('#tb_pro_collec #header .title {font-size :' + v.Size + 'px ;font-weight:' + (v.Bold ? 'bold' : 'normal') + '; color:' + v.Color + '}'); break;
                ...
                case 'tblPanelFooter':
                    switch (parseInt(v.Position)) {
                        case 1: style.append('#tb_pro_collec #footer .title {font-size :' + v.Size + 'px ;font-weight:' + (v.Bold ? 'bold' : 'normal') + '; color:' + v.Color + '}'); break;
                        case 2: style.append('#tb_pro_collec #footer .value {font-size :' + v.Size + 'px ;font-weight:' + (v.Bold ? 'bold' : 'normal') + '; color:' + v.Color + '}'); break;
                    }
                    break;
            }
        });
    ...
    function GetProductivity_Collec() {
        $.ajax({
            ...
            type: 'POST',
            data: JSON.stringify({ 'listLineId': Global.Data.LineId, 'tableTypeId': Global.Data.TableType }),
            contentType: 'application/json charset=utf-8',
            success: function (data) {
                if (data.Data != null) {
    ...
    function DrawContentAndSetData(datas) {
        ...
        //$('.value1').css('width', (parseFloat($('#tb_pro_collec #title2 .label1').css('width').replace('px', '')) - 2) + 'px');
        //$('.value2').css('width', (parseFloat($('#tb_pro_collec #title2 .label2').css('width').replace('px', '')) - 2) + 'px');
        //$('.value3').css('width', (parseFloat($('#tb_pro_collec #title2 .label3').css('width').replace('px', '')) - 2) + 'px');
        //$('.value4').css('width', (parseFloat($('#tb_pro_collec #title2 .label4').css('width').replace('px', '')) - 2) + 'px');
        //$('.value5').css('width', (parseFloat($('#tb_pro_collec #title2 .label5').css('width').replace('px', '')) - 2) + 'px');
    ...
    function SetDataWithoutDrawContent(datas) {
        $.each(datas, function (i, obj) {
            $('#tb_pro_collec #r_' + i + ' .value2').html(accounting.formatNumber(obj.THNgay) + '/' + accounting.formatNumber(obj.KHNgay));
            $('#tb_pro_collec #r_' + i + ' .value3').html(accounting.formatNumber(obj.THThang) + '/' + accounting.formatNumber(obj.KHThang));
            $('#tb_pro_collec #r_' + i + ' .value4').html(accounting.formatNumber(obj.DoanhThuTH) + '/' + accounting.formatNumber(obj.DoanhThuKH));
