...
GPRO.InsertProductivity = function () {
    ...
    function Save() {
        ...
        else if ($('#ht').val() == '4' && (lkCD + sl) > lkKCS && $('#Phase').val() != Global.Data.dongthungid && $('#phaseType').val() != 1)
    ...
    function GetPhaseDayInfo() {
        $.ajax({
            ...
            type: 'POST',
            data: JSON.stringify({ 'assignId': parseInt($('#product option:selected').attr('csp')), 'phaseId': $('#Phase').val() }),
            contentType: 'application/json charset=utf-8',
            beforeSend: function () { $('.progress').show(); },
            success: function (data) {
                $('.progress').hide();
